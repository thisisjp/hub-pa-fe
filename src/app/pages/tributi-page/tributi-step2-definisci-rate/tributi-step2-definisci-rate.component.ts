...
export class TributiStep2DefinisciRateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formGroup = this.formBuilder.group({
      ...
      postalIban: [defaultValues?.postalIban],
      postalAccountholder: [defaultValues?.postalAccountholder],
      postalAuthCode: [defaultValues?.postalAuthCode],
      abilitaCcPostale: [defaultValues?.abilitaCcPostale],
  ...
  isPercentageSecondaryDisabled(i: number): boolean {
    ...
    if (!this.isDateAfterConfigureServiceDate(dueDate?.value)) {
